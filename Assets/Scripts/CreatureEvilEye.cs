...
public class CreatureEvilEye : CreatureEnemy
{
    ...
    protected int CorrelateAttackDamage(int _level)
    {
        int result = 175;
        for (int i = 0; i <= _level; i++)
            result = Mathf.RoundToInt(result * (i > 0 ? (float)System.Math.Round((i + 5.5f) / (i * 9f) + 1, 2) : 1));
        return result;
    }
    ...
    public override void CorrelateLevelWithStats()
    {
        ...
        GetStateByName("view_dist").startStateValue = _level / 20 + 8;
        GetStateByName("protection_blunt").startStateValue = _level / 2 + 3;
        GetStateByName("protection_pricking").startStateValue = _level / 2 + 1;
        GetStateByName("protection_cutting").startStateValue = _level / 2 + 2;
        GetStateByName("protection_hewing").startStateValue = _level / 2 + 1;
        GetStateByName("protection_lashing").startStateValue = _level / 2 + 4;
        GetStateByName("protection_suffocative").startStateValue = _level / 2 + 2;
        GetStateByName("protection_scalding").startStateValue = _level / 2 + 2;
        GetStateByName("protection_freezing").startStateValue = _level / 2 + 0;
        GetStateByName("protection_electric").startStateValue = _level / 2 + 3;
        GetStateByName("protection_poison").startStateValue = _level / 2 + 2;
        minExperienceFromKilling = Mathf.RoundToInt(map.player.GetExperienceDiff(_level) * (_level > 0 ? ((_level + 2) / (_level * 65f)) : 1)) + 15;
        maxExperienceFromKilling = Mathf.RoundToInt(map.player.GetExperienceDiff(_level) * (_level > 0 ? ((_level + 2) / (_level * 50f)) : 1)) + 19;
    }
    ...
    public override void Attack()
    {
        ...
        if (map.cells[xToAttack, yToAttack].cellObject != null && map.cells[xToAttack, yToAttack].cellObject.GetComponent<Creature>() != null)
        {
            ...
            if (creatureToAttack.isAlive == true && creatureToAttack.GetStateValue("cur_health") > 0)
            {
                ...
                SetStateValue("attack_damage_bonus", GetStateValue("attack_damage_bonus") + 50);
                ChangeStateBonus("attack_damage", 50);
    ...
    {
        base.OnDeath();
        Creature c = GetCreatureInFront();
        if (c != null)
            c.GetDamage(GetStateValue("attack_damage"), autoattackDamageType, this);
        PlaySound("death");
    }
    ...
    public override void GetDamage(int _damage, DamageType damageType, Creature sender)
    {
        if (isAlive == true) // important condition
        {
            ...
            SetStateValue("attack_damage_bonus", 0);
            RefreshHealthbar();
    ...
    public void Die(Creature causedBy, bool useAfterdeathEffect)
    {
